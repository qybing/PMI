...
if __name__ == '__main__':
        start_url = 'http://www.66ip.cn/nmtq.php?getnum=300&isp=0&anonymoustype=0&start=&ports=&export=&ipaddress=&area=0&proxytype=1&api=66ip'
        html = get_page(start_url)
        a = html.split("var mediav_ad_height = '60';")[-1].split('<script type="text/javascript" src="http://www.66ip.cn/ggg/jquery.min.js"></script>')[0].split('</div>')[0].split('</script>')[-1]
        b = a.split('<br />')
        for i in b:
            result = i.strip()
            # yield result.replace(' ', '')
